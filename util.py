...
#     im_t = im_t.float().cpu()
#     im_t = torch.squeeze(im_t,dim=0).mul(255. / 255).clamp(0, 255).round()
#     im_np = np.transpose(im_t.numpy(), (1, 2, 0)).astype(np.uint8)
...
#     im_np = np.ascontiguousarray(im_np.transpose((2, 0, 1)))
#     im_np = torch.from_numpy(im_np).float()
#     im_np.mul_(255 / 255.)
#     im_np = torch.unsqueeze(im_np, dim=0).cuda()
