...
class Learner:
    ...
    def __init__(self, model):
        ...
        self.DARM_lr_scheduler = torch.optim.lr_scheduler.StepLR(model.optimizer_DARM, step_size=1000, gamma=0.5)

    def update(self, iteration, model):
        self.DARM_lr_scheduler.step()
        
        if iteration == 0:
            return
        # Update learning rate every update_l_rate freq
        if iteration % self.update_l_rate_freq == 0:
            for params in model.optimizer_G.param_groups:
                params['lr'] /= self.update_l_rate_rate
            for params in model.optimizer_D.param_groups:
                params['lr'] /= self.update_l_rate_rate
